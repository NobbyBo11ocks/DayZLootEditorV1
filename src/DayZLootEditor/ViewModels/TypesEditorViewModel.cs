...
public sealed class TypesEditorViewModel : ObservableObject
{
    ...
    private const string LootEditorFeature = "Loot Editor";
    ...
    private async Task OpenMissionFolderPathAsync(string folder)
    {
        ...
        await _recentFilesService.AddRecentMissionFolderAsync(folder).ConfigureAwait(true);
        RefreshRecentCollections();
        await RefreshCustomCeAsync().ConfigureAwait(true);

        var candidatePaths = new[]
        {
            Path.Combine(folder, "db", "types.xml"),
            Path.Combine(folder, "types.xml")
        };

        var typesPath = candidatePaths.FirstOrDefault(File.Exists);
        if (typesPath is null)
        {
            ClearLoadedFileState(folder, "Mission folder opened, but no db/types.xml was found. Use Open types.xml to choose it manually.");
            return;
        }

        await LoadFileAsync(typesPath, folder).ConfigureAwait(true);
...
private void RefreshCustomCePreview()
{
    ...
    {
        return string.Equals(workspace, CustomCeFeature, StringComparison.OrdinalIgnoreCase)
            ? CustomCeFeature
            : LootEditorFeature;
    }
